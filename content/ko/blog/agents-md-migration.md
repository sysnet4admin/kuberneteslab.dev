...
어느 모델에서도 B가 심볼릭 링크(C)보다 체계적으로 높지 않습니다. 4개 모델에서는 import가 3~4% 낮았고, Sonnet 5에서는 +6%였지만 같은 회차에서 심볼릭 링크도 똑같이 +6%였습니다. 심볼릭 링크는 OS가 링크만 풀어 주는, native와 달라질 수 없는 대조군입니다. 대조군과 함께 움직이는 편차는 전달 비용이 아니라 실행 편차입니다.
...
측정에서 import(B)가 이 대조군을 체계적으로 넘어선 적은 없습니다. 4개 모델에서 B는 native보다 3~4% 낮았고, Sonnet 5에서는 +6%였지만 대조군도 똑같이 +6%였습니다. 어느 쪽이든 전달 방식의 손해가 아니라는 점이 중요합니다.
