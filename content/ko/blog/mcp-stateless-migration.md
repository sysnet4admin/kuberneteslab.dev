...
description: "MCP 2026-07-28 스테이트리스 개정으로 세션 기반 서버를 옮기는 과정을 쿠버네티스에서 처음부터 끝까지 따라갑니다. 실제 요청과 응답, 매니페스트, 그리고 레플리카를 늘리고 파드를 죽여 가며 잰 수치도 함께 정리했습니다."
...
- 비교할 서버 2대. 구 스펙 쪽은 공식 `server-everything` npm 패키지의
...
두 서버에서 캡처를 실행합니다. 구 스펙과 신 스펙 각각 정해진 순서로 한 번씩 호출하고
요청과 응답을 원문 그대로 남깁니다.
...
물론 쿠버네티스에 익숙하신 분이라면 여기까지는 이미 알고 계신 내용일 수
있습니다. 파드는 언제든 종료되고 다른 노드로 옮겨질 수 있으니 상태를 파드 안에
두지 말라는 것이 쿠버네티스의 기본 설계 사상이고 구 스펙은 세션을 프로토콜
계층에 두다 보니 이 사상과 잘 맞지 않는 부분이 있었습니다. 그래서 이 글에서는
옮기는 방향이 맞는지가 아니라 옮기지 않고 두었을 때 실제로 얼마나 손해를
보는지를 재보려고 합니다. 처리량이 얼마나 떨어지는지, 어떤 형태로 실패하는지,
그리고 옮기고 나면 무엇을 다시 정해야 하는지를 확인해 보겠습니다. 여기서
스테이트풀셋(StatefulSet)을 떠올리실 수도 있는데 이 문제와는 해결해 주는 대상이
다릅니다. 스테이트풀셋은 파드의 이름을 고정하고 파드마다 저장 볼륨을 붙여
주지만 서비스(Service)가 요청을 어느 파드로 보내는지는 달라지지 않습니다.
게다가 메모리에 있는 세션은 그 볼륨에 저장되는 것이 아니라서 파드가 사라지면
세션도 똑같이 없어집니다.
...
것은 아닙니다. 세션이 가지고 있던 상태가 핸들로 이름만 바뀌었을 뿐, 그 값을
어디에 둘지는 여전히 만드는 사람이 정해야 하기 때문입니다.

## 4단계: 핸들 상태를 보관할 곳을 정합니다

설계는 두 가지로 나누어 볼 수 있습니다. 첫 번째는 값을
...
두 번째는 상태를 Redis에 두고 핸들은 불투명한 키로 두는 방식입니다.
...
나왔습니다. 어느 파드가 세션을 가지고 있고 내 연결이 어느 파드로 가느냐에 따라
결과가 나누어지기 때문입니다. 레플리카 1은 196.8에서 200.0으로 안정적이니 측정
잡음이 아니라 구 스펙의 성질입니다.
...
59건에서 1,072건까지 갈렸는데, 죽은 파드가 세션을 몇 개나 가지고 있었는지에
...
핸들 설계에도 같은 시험을 했습니다. 파드 메모리 구현은 파드가 종료되면 가지고 있던
...
호스트 1대라 클라이언트 IP가 하나이고 ClientIP 친화도는 IP 단위로
고정하므로 트래픽 전부가 파드 하나로 갔습니다. 세션이 전부 살아남은 대신 부하
분산을 전부 포기한 것입니다. 실제 환경은 클라이언트 IP가 여러 개라 효과가
부분적일 것이고 요청이 많은 클라이언트 1개가 파드 1개를 계속 누르게 됩니다.
...
1. 바꾸기 전에 구 스펙의 동작을 먼저 기록합니다. 새 연결에서 실패한 요청 1개를
   함께 남기면 전후 대조가 가장 분명해집니다.
2. v2 SDK로 옮겨 전송 계층을 포팅합니다. 이 단계에서 손볼 곳은 많지 않을
   것입니다.
3. 세션이 가지고 있던 상태를 하나도 빠짐없이 찾습니다. 여기서부터가 실제로 손이
   많이 가는 부분입니다.
4. 상태마다 정합니다. 모든 레플리카가 공유하는 키로 서명해 핸들에 담거나, 공유
   저장소에 둡니다. 파드 메모리에는 두지 않습니다. 쿠버네티스 워크로드가 세션을
   외부 스토어에 두는 것과 같은 원칙입니다.
